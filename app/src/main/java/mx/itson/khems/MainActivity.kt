...
class MainActivity : AppCompatActivity(),View.OnClickListener {
    ...
    fun iniciar() {
        ...
        for (i in 1..12) {
            btnSeleccion = findViewById<ImageButton>(resources.getIdentifier("opcion$i", "id", this.packageName))
    ...
    fun destapar(opcion: Int) {
        if (opcion == ubicacion) {
            ...
            var btnSeleccion = findViewById<View>(resources.getIdentifier("opcion$opcion", "id", this.packageName)) as ImageButton
            btnSeleccion.setBackgroundResource(R.drawable.icon_cheems)
            contador++
            puntaje++
            if (contador == 11) {
                ...
                val intent = Intent(this, NewPlayer::class.java).apply{
                    putExtra("juegos",juegos)
    ...
    override fun onClick(v: View?) {
        ...
        if(v?.id ==  R.id.btn_list){
            val intent = Intent(this,ListPlayersActivity::class.java)
            startActivity(intent)
        }
        when(v?.id){
            (R.id.opcion1)->{
